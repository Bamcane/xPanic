...
function NewConfig(on_configured_callback)
    ...
    if on_configured_callback then
        config.OnConfigured = on_configured_callback
    end
...
function DefaultOptionDisplay(option)
    ...
    if option.value == 1 or option.value == true then
        return "yes"
    end
    return option.value
end
...
function MakeOption(name, value, check, save, display, printhelp)
    ...
    if o.Display == nil then
        o.Display = DefaultOptionDisplay
    end
    return o
end
...
function OptCCompiler(name, default_driver, default_c, default_cxx, desc)
    ...
    local apply = function(option, settings)
        ...
        if option.c_compiler then
            settings.cc.c_compiler = option.c_compiler
        end
        if option.cxx_compiler then
            settings.cc.cxx_compiler = option.cxx_compiler
        end
    end
    ...
    local display = function(option)
        ...
        if option.c_compiler then
            s = s .. " c=" .. option.c_compiler
        end
        if option.cxx_compiler then
            s = s .. " cxx=" .. option.cxx_compiler
        end
        return s
    end
    ...
    if default_driver then
        o.driver = default_driver
    end
    if default_c then
        o.c_compiler = default_c
    end
    if default_cxx then
