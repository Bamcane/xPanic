	...
	case WEAPON_RIFLE:
	{
		...
		if (m_RiflePos != vec2(0, 0))
		{
			if (m_RiflePos == m_Pos)
			{
				...
				GameServer()->SendChatTarget(m_pPlayer->GetCID(), _("第二个标点不能设置在这里."));
				m_RiflePos = vec2(0, 0);
				return;
			}
...
bool CCharacter::GiveWeapon(int Weapon, int Ammo)
{
	if (Weapon == WEAPON_RIFLE && !m_aWeapons[Weapon].m_Ammo || Weapon != WEAPON_RIFLE && m_aWeapons[Weapon].m_Ammo < 5 || !m_aWeapons[Weapon].m_Got)
	{
		...
		if (Weapon == WEAPON_RIFLE)
			m_aWeapons[Weapon].m_Ammo = min((2), Ammo);
		else
			m_aWeapons[Weapon].m_Ammo = min(m_mAmmo + m_pPlayer->m_AccData.m_Ammo, Ammo);
...
void CCharacter::TickDefered()
{
	...
	if (Events & COREEVENT_GROUND_JUMP)
		GameServer()->CreateSound(m_Pos, SOUND_PLAYER_JUMP, Teams()->TeamMask(Team(), m_pPlayer->GetCID()));

	if (Events & COREEVENT_HOOK_ATTACH_PLAYER)
		GameServer()->CreateSound(m_Pos, SOUND_HOOK_ATTACH_PLAYER, Teams()->TeamMask(Team(), -1, m_pPlayer->GetCID()));
	if (Events & COREEVENT_HOOK_ATTACH_GROUND)
		GameServer()->CreateSound(m_Pos, SOUND_HOOK_ATTACH_GROUND, Teams()->TeamMask(Team(), m_pPlayer->GetCID(), m_pPlayer->GetCID()));
	if (Events & COREEVENT_HOOK_HIT_NOHOOK)
		GameServer()->CreateSound(m_Pos, SOUND_HOOK_NOATTACH, Teams()->TeamMask(Team(), m_pPlayer->GetCID(), m_pPlayer->GetCID()));
...
void CCharacter::ExperienceAdd(int Exp, int ClientID)
{
	...
	if (m_pPlayer)
		pPlayer->m_AccData.m_Exp += m_pPlayer->m_AccData.m_Freeze ? 0 : Exp;
	else
		pPlayer->m_AccData.m_Exp += Exp;

	if (pPlayer->m_AccData.m_Exp >= pPlayer->ExpNeedToNextLvl() && pPlayer && GameServer()->GetPlayerChar(ClientID))
	{
	...
	{
		char SendExp[64];
		str_format(SendExp, sizeof(SendExp), "经验 %d/%d", pPlayer->m_AccData.m_Exp, pPlayer->ExpNeedToNextLvl());
...
void CCharacter::HandleTiles(int Index)
{
	...
	{
		GameServer()->SendChatTarget(GetPlayer()->GetCID(), _("你获得了无限的跳跃"));
		m_SuperJump = true;
		if (m_Core.m_Jumps == 0)
		{
	...
	{
		GameServer()->SendChatTarget(GetPlayer()->GetCID(), _("你失去了无限跳."));
		m_SuperJump = false;
		if (m_Core.m_Jumps == 0)
		{
			m_NeededFaketuning |= FAKETUNE_NOJUMP;
...
void CCharacter::SendZoneMsgs()
{
	...
	if (m_TuneZoneOld >= 0 && GameServer()->m_ZoneLeaveMsg[m_TuneZoneOld]) // m_TuneZoneOld >= 0: avoid zone leave msgs on spawn
	{
		...
		GameServer()->SendChatTarget(m_pPlayer->GetCID(), _(cur));
	}
	// send zone enter msg
