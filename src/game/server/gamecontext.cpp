...
void CGameContext::SendPM(int ClientID, int FromID, char *string)
{
	if (!m_apPlayers[FromID])
		return SendChatTarget(ClientID, _("没有玩家用这个ID"));
	if (ClientID == FromID)
		return SendChatTarget(FromID, _("哥们以为能给自己发消息 (-_-')"));
			...
			if (str_comp_nocase(pMsg->m_Type, "option") == 0)
			{
				...
				while (pOption)
				{
					if (str_comp_nocase(pMsg->m_Value, pOption->m_aDescription) == 0)
					{
						...
						if (!m_apPlayers[ClientID]->m_Authed && (strncmp(pOption->m_aCommand, "sv_map ", 7) == 0 || strncmp(pOption->m_aCommand, "change_map ", 11) == 0 || strncmp(pOption->m_aCommand, "random_map", 10) == 0 || strncmp(pOption->m_aCommand, "random_unfinished_map", 21) == 0))
						{
							if (time_get() < m_LastMapVote + (time_freq() * g_Config.m_SvVoteMapTimeDelay))
							{
								char chatmsg[512] = {0};
